class HUD {
    ...
    createElements() {
        this.overlay.innerHTML = `
            ...
            <div class="hud-element hud-shield-bar" id="hud-shield-bar">
                ...
                <div class="hud-bar-text" id="hud-shield-text">装甲: 0/0</div>
            </div>
            ...
            <div class="hud-element hud-weapon" id="hud-weapon">炮型: 穿甲炮 Lv.1</div>
        ...
        this.dmgNumbersEl.addEventListener('click', () => {
            ...
            localStorage.setItem('tankbattalion_dmg_numbers', this._game.damageNumbersEnabled ? 'true' : 'false');
    ...
    update(game) {
        ...
        if (player.shield > 0) {
            ...
            this.shieldTextEl.textContent = `装甲: ${Math.ceil(player.shield)}/${player.maxShield}`;
        ...
        let weaponName = CONFIG.WEAPONS[player.weapon]?.name || '穿甲炮';
        this.weaponEl.textContent = `炮型: ${weaponName} Lv.${player.powerLevel}`;
    ...
    showBossBar(boss) {
        if (!document.getElementById('boss-hp-container')) {
            ...
            container.innerHTML = `
                <div class="boss-hp-label">⚠ BOSS: 钢铁巨兽 ⚠</div>
