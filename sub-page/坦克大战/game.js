...
(()=>{
  ...
  const startJoystick=event=>{if(!touchCapable||joystickTouchId!==null)return;const touch=event.changedTouches[0];if(!touch)return;joystickTouchId=touch.identifier;joystick.classList.add('active');updateJoy(touch.clientX,touch.clientY)};
  const moveJoystick=event=>{const touch=joystickTouchId===null?null:changedTouch(event,joystickTouchId);if(!touch)return;updateJoy(touch.clientX,touch.clientY)};
  const endJoystick=event=>{if(joystickTouchId===null||!changedTouch(event,joystickTouchId))return;clearJoy()};
  // touch-action:none 负责阻止滚动/缩放，因此监听器可以安全地使用 passive:true。
  joystick.addEventListener('touchstart',startJoystick,{passive:true});
  joystick.addEventListener('touchmove',moveJoystick,{passive:true});
  joystick.addEventListener('touchend',endJoystick,{passive:true});
  joystick.addEventListener('touchcancel',endJoystick,{passive:true});
  const startShoot=event=>{if(!touchCapable||shootTouchId!==null)return;const touch=event.changedTouches[0];if(!touch)return;shootTouchId=touch.identifier;shoot.classList.add('active');fireOnce()};
  const endShoot=event=>{if(shootTouchId===null||!changedTouch(event,shootTouchId))return;shootTouchId=null;shoot.classList.remove('active')};
  shoot.addEventListener('touchstart',startShoot,{passive:true});
  shoot.addEventListener('touchend',endShoot,{passive:true});
  shoot.addEventListener('touchcancel',endShoot,{passive:true});
