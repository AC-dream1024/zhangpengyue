...
// 移动端：使用上下左右方向键写入既有输入层，不改动战斗、碰撞和 AI 逻辑。
(()=>{
  const controls=document.createElement('div');
  controls.className='mobile-controls';
  controls.id='mobileControls';
  controls.innerHTML='<div class="direction-pad" role="group" aria-label="移动方向键"><button class="direction-key up" data-direction="arrowup" type="button" aria-label="向上移动">▲</button><button class="direction-key left" data-direction="arrowleft" type="button" aria-label="向左移动">◀</button><button class="direction-key down" data-direction="arrowdown" type="button" aria-label="向下移动">▼</button><button class="direction-key right" data-direction="arrowright" type="button" aria-label="向右移动">▶</button></div><button class="mobile-shoot" id="mobileShoot" type="button" aria-label="发射子弹"><span>●</span><small>射击</small></button>';
  ...
  const shoot=controls.querySelector('#mobileShoot');
  const directionButtons=[...controls.querySelectorAll('.direction-key')];
  const activeDirections=new globalThis.Map();
  const setDirection=(key,on)=>{if(on)keys.add(key);else keys.delete(key)};
  const releaseDirection=button=>{const key=activeDirections.get(button);if(key){setDirection(key,false);activeDirections.delete(button)}button.classList.remove('active')};
  const pressDirection=(button,event)=>{if(!touchCapable)return;event.preventDefault();const key=button.dataset.direction;if(activeDirections.has(button))return;activeDirections.set(button,key);setDirection(key,true);button.classList.add('active');button.setPointerCapture?.(event.pointerId)};
  directionButtons.forEach(button=>{button.addEventListener('pointerdown',event=>pressDirection(button,event));button.addEventListener('pointerup',()=>releaseDirection(button));button.addEventListener('pointercancel',()=>releaseDirection(button));button.addEventListener('pointerleave',event=>{if(activeDirections.has(button)&&event.pointerType==='mouse')releaseDirection(button)});});
  const clearDirections=()=>directionButtons.forEach(releaseDirection);
  ...
  const startShoot=event=>{if(!touchCapable||shootPointerId!==null)return;event.preventDefault();shootPointerId=event.pointerId;shoot.setPointerCapture?.(event.pointerId);shoot.classList.add('active');fireOnce()};
  const endShoot=()=>{shootPointerId=null;shoot.classList.remove('active')};
  shoot.addEventListener('pointerdown',startShoot);shoot.addEventListener('pointerup',endShoot);shoot.addEventListener('pointercancel',endShoot);
  window.addEventListener('blur',()=>{clearDirections();endShoot()});
  const originalShowMenu=Game.prototype.showMenu;
  Game.prototype.showMenu=function(...args){clearDirections();endShoot();return originalShowMenu.apply(this,args)};
})();
