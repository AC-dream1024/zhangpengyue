...
// 移动端：参考“坦克迷宫”提供虚拟摇杆和按住连发按钮。
(()=>{
  const controls=document.createElement('div');
  controls.className='mobile-controls';
  controls.id='mobileControls';
  controls.innerHTML='<div class="joystick" id="moveJoystick" aria-label="虚拟摇杆"><div class="joystick-ring"></div><div class="joystick-knob" id="joystickKnob"></div><span>移动</span></div><button class="mobile-shoot" id="mobileShoot" type="button" aria-label="射击"><span>●</span><small>射击</small></button>';
  document.querySelector('.screen-wrap').appendChild(controls);
  ...
  const mobileKeys=new Set();let joyPointer=null;
  const setKey=(key,on)=>{if(on){keys.add(key);mobileKeys.add(key)}else{keys.delete(key);mobileKeys.delete(key)}};
  const clearJoy=()=>{joyPointer=null;['arrowup','arrowdown','arrowleft','arrowright'].forEach(k=>setKey(k,false));knob.style.transform='translate(-50%,-50%)';joystick.classList.remove('active')};
  const updateJoy=(x,y)=>{const r=joystick.getBoundingClientRect(),cx=r.left+r.width/2,cy=r.top+r.height/2,max=r.width*.28;let dx=x-cx,dy=y-cy;const len=Math.hypot(dx,dy);if(len>max){dx=dx/len*max;dy=dy/len*max}knob.style.transform=`translate(calc(-50% + ${dx}px),calc(-50% + ${dy}px))`;['arrowup','arrowdown','arrowleft','arrowright'].forEach(k=>setKey(k,false));if(Math.hypot(dx,dy)>max*.22){if(Math.abs(dx)>Math.abs(dy))setKey(dx>0?'arrowright':'arrowleft',true);else setKey(dy>0?'arrowdown':'arrowup',true)}};
  joystick.addEventListener('pointerdown',e=>{if(!touchCapable)return;joyPointer=e.pointerId;joystick.setPointerCapture?.(joyPointer);joystick.classList.add('active');updateJoy(e.clientX,e.clientY);e.preventDefault()});
  joystick.addEventListener('pointermove',e=>{if(e.pointerId===joyPointer){updateJoy(e.clientX,e.clientY);e.preventDefault()}});
  joystick.addEventListener('pointerup',e=>{if(e.pointerId===joyPointer)clearJoy()});joystick.addEventListener('pointercancel',clearJoy);joystick.addEventListener('lostpointercapture',clearJoy);
  const stopShoot=()=>setKey(' ',false);shoot.addEventListener('pointerdown',e=>{if(!touchCapable)return;setKey(' ',true);shoot.classList.add('active');shoot.setPointerCapture?.(e.pointerId);e.preventDefault()});shoot.addEventListener('pointerup',()=>{stopShoot();shoot.classList.remove('active')});shoot.addEventListener('pointercancel',()=>{stopShoot();shoot.classList.remove('active')});shoot.addEventListener('lostpointercapture',()=>{stopShoot();shoot.classList.remove('active')});
  window.addEventListener('blur',()=>{clearJoy();stopShoot()});
  const originalShowMenu=Game.prototype.showMenu;
  Game.prototype.showMenu=function(...args){clearJoy();stopShoot();return originalShowMenu.apply(this,args)};
})();
