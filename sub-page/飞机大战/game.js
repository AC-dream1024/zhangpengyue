class Game {
    constructor(canvas) {
        ...
        this.storyProgress = parseInt(localStorage.getItem('starfighter_story_progress') || '0');
        this.isVictory = false;

        this.upgrades = this.loadUpgrades();
        this.totalPoints = parseInt(localStorage.getItem('starfighter_points') || '0');
        this.damageNumbersEnabled = localStorage.getItem('starfighter_dmg_numbers') !== 'false';
    ...
    setupResize() {
        ...
        window.addEventListener('orientationchange', () => {
            // orientationchange 在部分移动端先于视口尺寸更新触发，
            // 多次标记以覆盖慢速设备的尺寸更新延迟
    ...
    resize() {
        ...
        if (this.starfield) {
            this.starfield.resize(oldW, oldH, w, h);
        }
    ...
    // 统一暂停入口：所有场景的 P/ESC 键、HUD 暂停按钮都走这里
    togglePause() {
        ...
        if (pauseBtn) {
            if (paused) {
                pauseBtn.innerHTML = '<svg width="18" height="18" viewBox="0 0 20 20"><polygon points="5,3 17,10 5,17" fill="#00ffff"/></svg>';
            } else {
                pauseBtn.innerHTML = '<svg width="18" height="18" viewBox="0 0 20 20"><rect x="4" y="3" width="4" height="14" fill="#00ffff"/><rect x="12" y="3" width="4" height="14" fill="#00ffff"/></svg>';
            }
        }
    ...
    loadUpgrades() {
        try {
            return JSON.parse(localStorage.getItem('starfighter_upgrades') || '{}');
        } catch {
            return {};
        }
    }
    ...
    saveUpgrades() {
        localStorage.setItem('starfighter_upgrades', JSON.stringify(this.upgrades));
        localStorage.setItem('starfighter_points', this.totalPoints.toString());
    }
    ...
    advanceStage() {
        ...
        if (this.selectedMode === 'story') {
            ...
            if (this.currentStage >= this.storyProgress) {
                ...
                localStorage.setItem('starfighter_story_progress', this.storyProgress.toString());
            }
            // 触发关卡完成界面
    ...
    savePoints() {
        ...
        // 保存最高分
        const highScore = parseInt(localStorage.getItem('starfighter_highscore') || '0');
        if (this.score > highScore) {
            localStorage.setItem('starfighter_highscore', this.score.toString());
        }
    }
    ...
    drawDamageVignette(ctx) {
        ...
        gradient.addColorStop(0.7, `rgba(255, 0, 0, ${pulse * 0.5})`);
        gradient.addColorStop(1, `rgba(255, 0, 0, ${pulse})`);
        ctx.fillStyle = gradient;
        ctx.fillRect(0, 0, CONFIG.CANVAS.width, CONFIG.CANVAS.height);

        ctx.strokeStyle = `rgba(255, 30, 30, ${0.4 + intensity * 0.3})`;
    ...
    start() {
        ...
        const loop = (timestamp) => {
            try {
                // 尺寸变更：在绘制前同步处理，避免清空 canvas 后出现空白帧
                let resized = false;
