class Boss {
    ...
    drawBody(ctx) {
        ...
        // === 第一阶段：无光晕元素（shadowBlur=0）===
        ctx.shadowBlur = 0;

        // 主体填充
        ctx.fillStyle = this.isFlashing ? '#ffffff' : '#0a0a1a';
        ctx.strokeStyle = this.coreColor;
        ctx.lineWidth = 3;
        ctx.beginPath();
        ctx.moveTo(-hw, -hh);
        ctx.lineTo(-hw + 30, -hh + 10);
        ctx.lineTo(-hw + 40, hh - 10);
        ctx.lineTo(-hw + 20, hh);
        ctx.lineTo(hw - 20, hh);
        ctx.lineTo(hw - 40, hh - 10);
        ctx.lineTo(hw - 30, -hh + 10);
        ctx.lineTo(hw, -hh);
        ctx.lineTo(hw - 10, -hh + 20);
        ctx.lineTo(-hw + 10, -hh + 20);
        ctx.closePath();
        ctx.fill();
        ctx.stroke();
        ...
        ctx.fillRect(-hw + 15, -hh + 25, hw * 2 - 30, hh * 2 - 40);
        ...
        for (let i = -1; i <= 1; i++) {
            ctx.beginPath();
            ctx.arc(i * 60, hh - 15, 6, 0, Math.PI * 2);
            ctx.fill();
        }

        // === 第二阶段：带光晕元素（shadowBlur 设置一次）===
        ...
        ctx.beginPath();
        ctx.arc(0, 0, 10 * corePulse, 0, Math.PI * 2);
        ctx.fill();
        ...
        if (this.phase >= 2) {
            ...
            ctx.fillRect(-hw + 10, -10, 15, 20);
            ctx.fillRect(hw - 25, -10, 15, 20);
        }
