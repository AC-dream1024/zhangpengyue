class MenuScene extends Scene {
    ...
    createMenu() {
        ...
        const shipTypes = CONFIG.SHIP_TYPES;
        ...
        for (const key in shipTypes) {
            ...
            const active = game.selectedShip === ship.id ? 'active' : '';
            shipHTML += `
                <div class="ship-card ${active}" data-ship="${ship.id}" style="--ship-color:${ship.color};">
                    <div class="ship-card-icon">${ship.icon}</div>
                    <div class="ship-card-name">${ship.name}</div>
                    <div class="ship-card-codename">${ship.codename}</div>
                    <div class="ship-card-advantage">${ship.advantage}</div>
                    <div class="ship-card-desc">${ship.description}</div>
        ...
        if (game.storyProgress > 0 && stages.length > 0) {
            progressHTML = `<div class="story-progress">闯关进度: 已解锁 ${game.storyProgress + 1} / ${stages.length} 关</div>`;
        }
        ...
        this.ui = this.createUI(`
            ...
            <button class="btn btn-secondary" id="btn-shop">🛒 技能商店</button>
        ...
        document.getElementById('btn-reset').addEventListener('click', () => {
            if (confirm('确定要重置最高纪录吗？')) {
                localStorage.setItem('starfighter_highscore', '0');
                this.enter();
            }
        });
    }
